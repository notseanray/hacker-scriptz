...
update_weather () {
	weather_api="$(curl --connect-timeout 1 -s "wttr.in?format=1"| sed -E "s/^(.).*\+/\1/")"
	# dash does not support &&
	if [ ${#weather_api} -gt 1 ]; then
		if [ ${#weather_api} -lt 10 ]; then
			weather="$weather_api | "
		fi
	else
		weather=""
...
update_btc () {
	btc_api="$(curl --connect-timeout 1 -s http://api.coindesk.com/v1/bpi/currentprice.json | jq -r .bpi.USD.rate)"
